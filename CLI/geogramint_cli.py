...
from Db import groups_collection, users_collection
from Db.orm import CollectionDocsInserter, insert_scanning_results_object
...
def start_scan(lat: float, lon: float, output_json: str = typer.Option("cache_telegram", help="Directory Path"),
    ...
    users_list: list[dict] = [ob.__dict__() for ob in users]
    groups_list: list[dict] = [ob.__dict__() for ob in groups]

    if output_json[-1] == '/':
        output_json = output_json[:-1]
    json_string_user = json.dumps(users_list, ensure_ascii=False)
    with codecs.open(output_json + '/users.json', 'w', 'utf-8') as f:
        f.write(json_string_user)
    json_string_group = json.dumps(groups_list, ensure_ascii=False)
    ...
    groups_inserter: CollectionDocsInserter = CollectionDocsInserter(groups_collection)
    users_inserter: CollectionDocsInserter = CollectionDocsInserter(users_collection)
    groups_ids = groups_inserter.insert_docs_and_get_ids(groups_list)
    users_ids = users_inserter.insert_docs_and_get_ids(users_list)
    insert_scanning_results_object(users_ids, groups_ids)
    console.print("[cyan]Results of search saved to db")
...
# Была помечена как эксперементальная разработчиком Geogramint в DOCS (ниже этого коммента) и отключена
